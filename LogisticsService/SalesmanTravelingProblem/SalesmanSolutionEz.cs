    ...
    public class SalesmanSolutionEz
    {
            ...
            out Order[] alwaysUndeliveredOrders)
        {
            ...
            try
            {
                ...
                alwaysUndeliveredOrders = orderCluster[3].ToArray();
                if (alwaysUndeliveredOrders != null && alwaysUndeliveredOrders.Length > 0)
                {
                    Logger.WriteToLog($"[debug] ClassifyOrders. // 7 orders =({Helper.ArrayToString(alwaysUndeliveredOrders.Select(p => p.Id).ToArray())})");
                }
            ...
            out CourierDeliveryInfo[] assembledOrders, out CourierDeliveryInfo[] receiptedOrders, out Order[] undeliveredOrders, out Order[] tabuOrders)
        {
            ...
            try
            {
                ...
                if (isOnTime && !isBehindTime)
                {
                    ...
                    {
                        Logger.WriteToLog($"[debug] SalesmanSolutionEy.CreateShopDeliveries.CreateShopDeliveries_OnTime // 4 rc1 = {rc1} orders =({Helper.ArrayToString(undeliveredOrders.Select(p => p.Id).ToArray())})");
                    }
                }
                ...
                else if (isOnTime && isBehindTime)
                {
                    ...
                    if (undeliveredOrders != null && undeliveredOrders.Length > 0)
                    {
                        Logger.WriteToLog($"[debug] SalesmanSolutionEy.CreateShopDeliveries.CreateShopDeliveries_OnTimeAndBehindTime // 4 rc1 = {rc1} orders =({Helper.ArrayToString(undeliveredOrders.Select(p => p.Id).ToArray())})");
                    }
        ...
        private int CreateShopDeliveriesEx(Shop shop, Order[] shopOrders, Courier shopCourier, bool isLoop, DateTime calcTime, out CourierDeliveryInfo[] allPossibleDeliveries)
        {
            ...
            try
            {
                ...
                {
                    rc1 = CreateShopDeliveries8(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                ...
                else if (orderCount <= orderLimitsForPathLength[7])
                {
                    ...
                    rc1 = CreateShopDeliveries7(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }
                else if (orderCount <= orderLimitsForPathLength[6])
                {
                    ...
                    rc1 = CreateShopDeliveries6(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }
                else if (orderCount <= orderLimitsForPathLength[5])
                {
                    ...
                    rc1 = CreateShopDeliveries5(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }
                else if (orderCount <= orderLimitsForPathLength[4])
                {
                    ...
                    rc1 = CreateShopDeliveries4(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }
                else if (orderCount <= orderLimitsForPathLength[3])
                {
                    rc1 = CreateShopDeliveries3(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                    //return CreateShopDeliveries3(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }
                else
                {
                    rc1 = CreateShopDeliveries2(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                    //return CreateShopDeliveries2(shop, courierOrders, shopCourier, isLoop, calcTime, out allPossibleDeliveries);
                }

                rc = (rc1 == 0 ? 0 : 100 * rc + rc1);
                return rc;
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                if (rc != 0)
                {
                    if (shopOrders == null)
                    {
                        Logger.WriteToLog($"[debug] CreateShopDeliveriesEx. finnaly rc = {rc}");
                    }
                    else
                    {
